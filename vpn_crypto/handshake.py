...
PSK_SIZE = 32
PSK_ENV_VAR = "VPN_PSK_HEX"
_DEFAULT_LAB_PSK = b"\x22" * PSK_SIZE
...
def load_lab_psk() -> bytes:
    """Carga la PSK desde el entorno; usa fallback solo para laboratorio local."""

    psk_hex = os.getenv(PSK_ENV_VAR)

    if not psk_hex:
        return _DEFAULT_LAB_PSK

    try:
        psk = bytes.fromhex(psk_hex)
    except ValueError as error:
        raise ValueError(
            f"{PSK_ENV_VAR} debe ser hexadecimal valido"
        ) from error

    if len(psk) != PSK_SIZE:
        raise ValueError(
            f"{PSK_ENV_VAR} debe representar exactamente 32 bytes"
        )

    return psk


LAB_PSK = load_lab_psk()
...
def create_handshake_transcript(
    client_public_key: bytes,
    server_public_key: bytes,
) -> bytes:
    if len(client_public_key) != X25519_KEY_SIZE:
        raise ValueError("La clave publica del cliente debe medir 32 bytes")

    if len(server_public_key) != X25519_KEY_SIZE:
        raise ValueError("La clave publica del servidor debe medir 32 bytes")
...
def derive_session_key(
    ...
    if len(peer_public_key_bytes) != X25519_KEY_SIZE:
        raise ValueError("La clave publica remota debe medir 32 bytes")
